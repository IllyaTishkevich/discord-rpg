...
- PvP (`/duel`) — живой синхронный бой между двумя реальными игроками (см. `docs/BATTLE_ROOM_DESIGN.md`): `Battle` в статусе `waiting` — это и есть приглашение, раунд не считается, пока обе стороны не пришлют свои цели действия через `/submit-actions`. Турнирные матчи (`/tournament`) — отдельный, более простой случай: сервер симулирует их сразу целиком по статам обоих персонажей (см. `TournamentService`), без живого пошагового боя.

 ...
 * IncreaseCapacity only make sense on a worn item (effect derived live from
 * "is this equipped right now" — see Character::getAllBits()/getAllAbilities()/
 * getInventoryCapacity()); Heal/RestoreEnergy/GrantXp only make sense on a
 * consumable, applied once by InventoryService::useItem() and then the item
 * is gone.
 */
enum ItemEffectType: string
{
    ...
    {
        return match ($this) {
            self::AddBit, self::AddAbility, self::IncreaseCapacity => true,
